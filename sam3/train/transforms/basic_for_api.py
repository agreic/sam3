...
class CLAHETransformAPI:
    """
    Apply CLAHE (Contrast Limited Adaptive Histogram Equalization) to each image
    in a datapoint *before* tensor conversion.

    This is intended to be used in pipelines like:
        CLAHETransformAPI(...) -> RandomResizeAPI(...) -> ToTensorAPI() -> NormalizeAPI(...)
    where `datapoint.images[i].data` is a PIL image when this transform runs.
    """
    ...
    def __call__(self, datapoint: Datapoint, **kwargs):
        # If there are no images, just passthrough
        if not hasattr(datapoint, "images") or not datapoint.images:
            return datapoint

        # Lazy import to avoid hard dependency if cv2 is not used elsewhere
        import cv2

        for img in datapoint.images:
            # We expect a PIL image here; if it's something else, skip gracefully
            if not isinstance(img.data, PILImage.Image):
                continue

            img_np = np.array(img.data)
            ...
            if img_np.dtype == np.uint16:
                ...
                if img_max > img_min:
                    img_np = (
                        255.0 * (img_np - img_min) / (img_max - img_min + 1e-6)
                    ).astype(np.uint8)
                else:
                    img_np = img_np.astype(np.uint8)

            # Apply CLAHE in LAB (for RGB) or grayscale
            ...
            # Write back as PIL so that downstream ToTensorAPI behaves as expected
            img.data = PILImage.fromarray(img_np)
